...
public class Q376_Wiggle_Subsequence {
	public int wiggleMaxLength(int[] nums) {
		...
		int result = 2;
		boolean smallReg = nums[k]<nums[k+1]; //to check series starting pattern
		for(int i=k+1; i<nums.length-1; i++) {
			if(smallReg&&nums[i+1]<nums[i]) {
				nums[result] = nums[i+1];
				result++;
				smallReg = !smallReg; //toggle the requirement from big to small number
			}else {
				if (!smallReg && nums[i + 1] > nums[i]) {
					nums[result] = nums[i + 1];
					result++;
