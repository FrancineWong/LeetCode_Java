...
public class Q096_Unique_Binary_Search_Trees {
	 ...
	 * number i in the sequence, and use the number as the root, naturally, the subsequence 1…(i-1) on its left side 
	 * would lay on the left branch of the root, and similarly the right subsequence (i+1)…n lay on the right branch 
	 * of the root. We then can construct the subtree from the subsequence recursively. Through the above approach, 
...
In terms of calculation, we need to start with the lower number, since the value of G(n) depends on the values 
of G(0) … G(n-1).
