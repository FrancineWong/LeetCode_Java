...
public class General_approach_to_backtrack {
/************************SubSet************************************************/
	...
	private void subset(List<List<Integer>> list , List<Integer> tempList, int [] nums, int start){
...
/********************Subset contains duplicate********************************/
	...
	private void combinationSum(List<List<Integer>> list, List<Integer> tempList, int [] nums, int remain, int start){
	    if(remain < 0) return;
	    else if(remain == 0) list.add(new ArrayList<>(tempList));
	    else{ 
	        for(int i = start; i < nums.length; i++){
	            tempList.add(nums[i]);
	            combinationSum(list, tempList, nums, remain - nums[i], i); // not i + 1 because we can reuse same elements
	            tempList.remove(tempList.size() - 1);
	        }
	    }
	}

	/**********************Combine sum no duplicate**********************************************************/	
	...
	private void combinationSum2(List<List<Integer>> list, List<Integer> tempList, int [] nums, int remain, int start){
	    if(remain < 0) return;
	    else if(remain == 0) list.add(new ArrayList<>(tempList));
	    else{
	        for(int i = start; i < nums.length; i++){
