...
public class Q064_Minimum_Path_Sum {
	 ...
	 * and sum[i][j - 1] (at the left column). So we need not maintain the full m*n matrix. Maintaining two 
	 * columns is enough and now we have the following code.*/
	public int minPathSum_0(int[][] grid) {
	        int m = grid.length;
	        int n = grid[0].length;
	        int[] pre = new int[m];
	        for( int i=0; i<m; i++) pre[i] = grid[0][0];
	        int[] cur = new int[m];
	        for (int i = 1; i < m; i++)
...
//	            swap(pre, cur); 
	 ...
	 * which is simply cur[i] before its update. So it is enough to use only one vector. Now the space 
	 * is further optimized and the code also gets shorter.*/
	public int minPathSum_1(int[][] grid) {
	        int m = grid.length;
	        int n = grid[0].length;
	        int[] cur = new int[m];
	        for( int i=0; i<m; i++) cur[i] = grid[0][0];
	        for (int i = 1; i < m; i++)
